...
function reportProblem(_ref) {
	...
	context.report({
		...
		fix: function fix(fixer) {
			...
			var sourceDecorators = source.node.decorators || [];
			var sourceText = [];
			for (var i = 0; i < sourceDecorators.length; i++) {
				var decoratorComment = sourceCode.getCommentsBefore(sourceDecorators[i]).slice(-1).pop();
				var hasNext = i < sourceDecorators.length - 1;
				if (decoratorComment) {
					fixes.push(fixer.remove(decoratorComment));
					sourceText.push('' + sourceCode.getText(decoratorComment) + determineNodeSeperator(decoratorComment, sourceDecorators[i]));
				}
				fixes.push(fixer.remove(sourceDecorators[i]));
				sourceText.push('' + sourceCode.getText(sourceDecorators[i]) + determineNodeSeperator(sourceDecorators[i], hasNext ? sourceDecorators[i + 1] : source.node));
			}
			if (sourceJSDoc) {
				fixes.push(fixer.remove(sourceJSDoc));
				sourceText.push('' + sourceCode.getText(sourceJSDoc) + determineNodeSeperator(sourceJSDoc, source.node));
			}

			fixes.push(fixer.remove(source.node));
			sourceText.push('' + sourceCode.getText(source.node) + determineNodeSeperator(source.node, sourceAfterToken));
...
function getMemberInfo(node, sourceCode) {
	...
	return { name: name, type: type, static: node.static, async: async, kind: node.kind, propertyType: propertyType, node: node };
}
...
var comparers = [{ property: 'name', value: 100, test: function test(m, s) {
	...
	} }, { property: 'async', value: 10, test: function test(m, s) {
